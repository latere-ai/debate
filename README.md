...
- **Forked debate, untouched root.** Each critic gets its own fork (`claude --fork-session`). The root session's transcript is never modified; the user resumes from where they left off when debate ends.
...
- **Gated on upstream research.** Implementation should not start until [agents-byzantine-tolerance](https://github.com/changkun/agents-byzantine-tolerance) spec 07a measures critic-found-bug rate ≥ 60% on seeded bugs.
