...
func Decide(s *round.Summary) SurfacingDecision {
	...
	case round.TermSteadyState:
		...
		return SurfacingDecision{
			Surface:    true,
			StdoutLine: fmt.Sprintf("[debate] %d unresolved; see %s", s.Unresolved, summaryPath),
			ExitCode:   1,
		}
	case round.TermInterrupted:
		return SurfacingDecision{
			Surface:    true,
			StdoutLine: fmt.Sprintf("[debate] interrupted (%d known unresolved); partial review at %s", s.Unresolved, summaryPath),
			ExitCode:   130,
		}
	default: // max-turn, cost-cap, malformed-output
		return SurfacingDecision{
			Surface:    true,
			StdoutLine: fmt.Sprintf("[debate] terminated %s (%d unresolved); see %s", s.Termination, s.Unresolved, summaryPath),
			ExitCode:   1,
		}
	}
}
