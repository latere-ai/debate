...
// and vitest mocks `*.css?raw` to empty — so read the stylesheet through node
// builtins (present at runtime). Imports go through string-variable specifiers
// so TS doesn't try to resolve node module types.
...
describe('dialectic.css dark spine-stop', () => {
  const genericDark = css.indexOf(
    '[data-theme="dark"] .v-dialectic .spine-stop {',
  );
  const isCDark = css.indexOf(
    '[data-theme="dark"] .v-dialectic .spine-stop.is-c',
  );
