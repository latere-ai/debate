...
Single root command (`agon`); no subcommands in v0. (`agon resume <id>` is v1, see [01-overview.md](01-overview.md) §Versioning.)
...
|---|---|---|---|---|
| `--main` | string | `claude` | `AGON_MAIN` | proposer agent: `claude` or `codex`. v0 errors on `codex` (see [06](06-preflight.md)). |
| `--side` | string | `codex` | `AGON_SIDE` | critic agent: `claude` or `codex`. |
| `--side-count` | int | `4` | `AGON_SIDE_COUNT` | number of critic forks. Must equal `len(--aspect)`. |
| `--main-model` | string | `""` | `AGON_MAIN_MODEL` | optional cross-family; required + must differ from `--side-model` same-family. |
| `--side-model` | string | `""` | `AGON_SIDE_MODEL` | same rule. |
| `--max-turn` | int | `6` | `AGON_MAX_TURN` | per-fork cap (P+C exchanges). |
| `--aspect` | stringSlice | `functional-logic,security,code-quality,performance` | `AGON_ASPECT` | comma-separated. Names are free-form (see [15](15-aspect-prompts.md)). |
| `--session-id` | string | `""` | `AGON_SESSION_ID` | claude root session id. Required for claude-as-proposer auto-trigger. |
| `--transcript` | string | `""` | `AGON_TRANSCRIPT` | path to root JSONL. Optional; preferred when present. |
| `--diff-from` | string | `HEAD` | `AGON_DIFF_FROM` | git ref for diff base. |
| `--diff-to` | string | `.` | `AGON_DIFF_TO` | `.` means working tree; otherwise a git ref. |
| `--task-context` | string | `""` | `AGON_TASK_CONTEXT` | mandatory iff neither `--session-id` nor `--transcript` is set. |
| `--judge` | string | `none` | `AGON_JUDGE` | one of `none`, `llm`, `human`. v0 only accepts `none`. |
| `--cost-cap` | int | `50000` | `AGON_COST_CAP` | total token budget across all forks. |
...
- [x] `agon --help` lists all flags with the table's defaults.
- [x] `agon --version` prints version/commit/date.
- [x] Every flag has a `AGON_*` env var that, when set, populates the corresponding `Flags` field if no CLI flag is given.
