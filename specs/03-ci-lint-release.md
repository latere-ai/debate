...
CI fails if any of: `go vet`, `golangci-lint run`, `go test ./...`, or `go build ./cmd/debate` fails.
...
go build -ldflags "\
  ...
  -o bin/debate ./cmd/debate
```

`debate --version` prints `debate <version> (<commit>, <date>)` and exits 0. The flag is wired in [04](04-cli-flags.md) and consumes these vars.
...
- `bin/debate --version` prints a non-`dev` string when built via the `make build`/`goreleaser` path.
